...
> **최종 업데이트**: 2026-07-13: §5 지식 그래프 파이프라인(Neo4j 적재 + **Bloom 시각화**) 및 §6 모델 구성 비교표(ATLOP 논문 vs Baseline 재현 vs RoBERTa+PU) 추가, Plan을 §7로 재번호.
>
...
- **선정 이유:** (a) 성능이 충분히 강하고 재현이 쉬워 **공정한 비교 기준**이 되며, (b) 우리가 개선하려는 지점이 구조적으로 명확히 드러난다 — 아래 §3의 세 문제는 모두 이 재현 모델에서 **실측으로 진단**한 것이다.
...
- **해결 방향:** 관련 논문 분석을 바탕으로 문맥 결합 방식을 개선해 low-attention 근거의 유실을 방지하고(예: 곱 연산의 완화·보완 신호 결합), 기존 성능과 비교 검증한다 (담당 트랙에서 설계·구현 진행 중).
...
- **해결 (`losses.py`의 `PUATLoss`):** distant의 Na를 확정 음성이 아닌 **미표기(unlabeled)**로 취급하는 PU 관점 — all-Na 쌍의 TH-랭킹 항만 `na_weight`로 다운웨이트. distant 사전학습 단계에만 적용하고 annotated 단계는 표준 ATLoss 유지(annotated의 Na는 gold). **A/B 검증 완료**(distant 5,000개, stage-1): dev F1 44.63→46.10(+1.47), Recall +10.09p, 임계값에 눌려 있던 정답 1,415개 구출(순이득 +1,238).

### 3.2 Baseline 대비 무엇이 다른가

| 문제 | Baseline (ATLOP) | Proposed | 검증 상태 |
|---|---|---|---|
| ① multi-hop 추론 | 쌍별 독립 분류 — 조합 경로 없음 | Entity Pair Graph + GCN(개선 1) / GAT(개선 2) 전파 | 구현·CPU 검증 완료, 풀 학습 진행 중 |
| ② low-attention 유실 | attention 곱 기반 LCP — 낮은 attention 근거 소실 | 문맥 결합 방식 개선으로 유실 방지 | 설계·구현 진행 중 |
| ③ Adaptive Thresholding | distant의 가짜 Na가 TH 학습 오염 | PUATLoss — 미표기 쌍의 TH-랭킹 항 다운웨이트 | subset A/B 완료(+1.47 F1), 풀 파이프라인 예정 |
...
- **E2 (문제 ①).** 개선 1(GCN)·개선 2(GAT)를 baseline과 **동일 레시피**로 학습·비교 — GCN vs GAT ablation 내장, multi-hop probe 재실행으로 정성 확인
- **E3 (문제 ②).** low-attention 유실 방지 개선의 구현 및 A/B — 장거리 probe 재실행 + inter-sentence F1로 확인
- **E4 (문제 ③).** PUATLoss 풀 파이프라인 A/B(distant 20,000 + annotated fine-tune, baseline 레시피와 동일 조건), `na_weight` sweep(0.3/0.5/0.7)
- **E5. 통합 및 정성 분석** — 검증된 개선들의 결합 모델 평가, 오류 유형 분류(특히 inter-sentence·multi-hop·희소 관계), probe 3종 종합 리포트
...
- **지표 검증 방법**
  ...
  - *신뢰성:* seed 변동(±1점 수준, PRD §5)을 감안해 해석하고 최종 비교는 복수 seed 평균 권장, DocRED **공식 스코어러 포팅본**으로 채점
...
§1.3의 최종 산출물(문서별 triple 집합)을 실제 지식 그래프로 적재해, "비정형 텍스트 → 구조화 지식"이라는 비즈니스 니즈(§1.1)의 **전 과정을 눈으로 확인 가능한 데모**로 완성한다.
...
1. **데모:** 임의 문서 → 추출 triple → 그래프 탐색/검색 — 최종 산출물이 KG·RAG·추론 시스템의 입력으로 직결됨(§1.4)을 실물로 시연.
2. **정성 검증 도구:** §4.3의 sanity check(관계 방향·엔티티 타입 정합성 위반)를 그래프 상에서 육안으로 확인 — 표 형태 예측 목록보다 오류 패턴이 즉시 드러난다.
...
- 산출물: Neo4j 적재 스크립트 + 발표용 Bloom 씬(장면)

---

## 6. 부록: 모델 구성 비교 (ATLOP 논문 vs Baseline 재현 vs RoBERTa+PU)

문제 ③ 트랙의 구현체(RoBERTa + PU)까지 포함한 상세 구성 비교. 문제 ①(GCN/GAT)·② 개선 모델의 열은 풀 학습 완료 후 추가 예정.
...
> 해석: Baseline 재현이 논문을 +0.62/+0.64 상회하며 재현 성공. RoBERTa+PU는 Baseline 대비 +0.06/+0.12로 시드 변동(±1점) 안 — 단독 구성으로는 사실상 동률이며, PU의 효과는 동일 조건 A/B(§3.1 문제 ③, distant 단계 +1.47 F1·Recall +10.09p)에서 분리 검증되었다.
...
| 7/15 – 7/16 | Ablation·하이퍼파라미터 탐색, 검증된 개선 결합(E5), 예측 triple Neo4j 적재·Bloom 시각화 구축(§5) | 실험 표, 학습 곡선, KG 적재 스크립트 |
| 7/17 – 7/19 | 결과 정리, 정성 분석(probe 종합 + Bloom 전/후 그래프 비교), 발표자료 작성 | Neo4j Bloom KG 데모, PPT |
| 7/20 | 발표 자료 정리 | 최종 발표 |
