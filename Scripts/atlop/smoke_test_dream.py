"""CPU wiring check for DocREModelDREAM (baseline + DREAM evidence-guided context).

Random-init tiny BERT on a few dev docs — proves the pipeline wires up, NOT
accuracy. Checks:
  1. features      - build_features_full gives sent_pos + gold evidence.
  ...
  4. evidence loss - evi_lambda on vs off changes the total (DREAM 항이 기여).

Run:  python -m Scripts.atlop.smoke_test_dream
"""
...
def tiny_model(tokenizer, evi_lambda):
    ...
    torch.manual_seed(0)
    return DocREModelDREAM(config, BertModel(config, add_pooling_layer=False),
...
def main():
    ...
    print(f"[features] doc0: sents={n_sent} pairs={len(f0['hts'])} pairs_with_evidence={n_evi}")
    ...
    # 2) evidence distribution sanity
    ...
    sums = p0.sum(1)
    assert torch.allclose(sums, torch.ones_like(sums), atol=1e-4), f"p_evi must sum to 1, {sums[:3]}"
    ...
    print(f"[fwd/bwd] loss={loss.item():.4f}  all params have grad (preds {tuple(preds.shape)})")
    ...
    print(f"[evidence loss] evi_lambda 0.5 vs 0.0: {loss_on.item():.4f} vs "
          f"{loss_off.item():.4f}  (d={loss_on.item() - loss_off.item():+.4f})")

    print("\nDREAM SMOKE TEST PASSED - baseline + evidence-guided context wired end-to-end.")
