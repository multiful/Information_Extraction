...
> **최종 업데이트**: 2026-07-16 (**Graph-grounded Evidence Retrieval 폴백 추가
> — 문제 8 추가 개선**): 사용자 제안(Graph Search → 답 없음 → 방문 노드 evidence
> 재검색 → LLM → Answer) 실측 검증 후 적용. `answer_with_subgraph()`가 "모름"을
> 낼 때만, 구조화 triple 없이 방문 노드들의 evidence 원문만 모아 재시도하는
> `_answer_with_evidence_only()` 추가 — 16회 반복 샘플링(AirAsia Zest 합병
> 질문)으로 검증: 1단계만 쓰면 정답 6/모름 8/오답 2였는데, "모름" 8건 중 **7건이
> 정답으로 복구**(정답 6→14). 오답 2건은 애초에 "모름"이 아니라 폴백이 안 걸려서
> 그대로 남음(설계상 의도된 범위). `majority_vote_answer()`가 `answer_with_subgraph()`를
> 감싸는 구조라 다수결에도 자동으로 적용됨 — 실제로 최종 답변이 "2013년 결성 →
> 2016년 AirAsia Philippines 합병"까지 구분하는 더 정교한 답으로 개선됨. 비용은
> "모름"일 때만 추가 호출이라 낮음.
>
> 이전 (2026-07-16, **문제 8 반복 샘플링+다수결 자동화 완료**):
...
**AirAsia Zest 케이스에 대한 정직한 한계 (다수결만으로는)**: 다수결 로직 자체는 정상 작동(2013년/2016년/모름을 정확히 별개 그룹으로 분리 확인)하지만, 이 질문은 6회 샘플링에서 정답 2/모름 2/오답 2로 **진짜 3분의 1씩 고르게 갈리는 케이스**라 n=3 다수결로는 종종 동점이 나 확실히 못 고침. 다른 BFS 질문(Cirit/Roketsan/Outotec/Manila/Philippines 등)은 전부 3표 만장일치로 안정적 — 이 케이스처럼 원래도 거의 균등하게 흔들리는 예외적인 질문에만 한계가 남음.

**Graph-grounded Evidence Retrieval 폴백 추가로 대폭 개선 (2026-07-16)**: 사용자 제안 — Graph Search → 답 없음 → 방문 노드 evidence 원문만 재검색 → LLM → Answer. `_answer_with_evidence_only()`로 구현, `answer_with_subgraph()`가 "모름"을 낼 때만 자동 호출(비용 절약). 16회 반복 샘플링 검증: 1단계(triple 기반)만 쓰면 정답 6/모름 8/오답 2였는데, "모름" 8건 중 **7건이 정답으로 복구**(정답 6→14, 87.5%). "모름"은 사실은 있는데 구조화된 triple 형태로는 LLM이 잘 못 읽어내는 경우가 대부분이라, 원문 문장만 다시 보여주면 잘 찾아냄. **오답 2건은 이 폴백으로도 안 고쳐짐** — 애초에 "모름"이 아니라 확신에 찬 오답이라 폴백 자체가 안 걸리기 때문(설계상 의도된 범위 밖, 다수결이 이 부분을 커버). `majority_vote_answer()`가 `answer_with_subgraph()`를 감싸는 구조라 다수결에도 자동 적용됨 — 실제로 최종 답이 "2013년 결성 → 2016년 AirAsia Philippines 합병"까지 구분하는 더 정교한 답으로 개선됨.
